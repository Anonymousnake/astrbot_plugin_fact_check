...
class FactCheckMetricsStore:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if not isinstance(data, dict):
            return loaded
        for key in ("requests", "success", "partial", "failure", "cache_hits", "followups"):
    ...
    def record(
        ...
        *,
        ...
        normalized = outcome if outcome in {"success", "partial", "failure"} else "failure"
        with self._lock:
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            result["average_seconds"] = round(float(result["elapsed_total"]) / requests, 2)
            return result

    def render_status(self) -> str:
        metrics = self.snapshot()
        stages = metrics["failure_stages"]
        stage_text = "、".join(f"{key}={value}" for key, value in sorted(stages.items())) or "无"
        return (
            "事实核查状态\n"
